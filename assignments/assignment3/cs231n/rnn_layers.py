...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h = np.tanh(at)
    cache = (x, prev_h, Wx, Wh, at, next_h)
...
def rnn_step_backward(dnext_h, cache):
    ...
    x, prev_h, Wx, Wh, at, next_h = cache
    dat = dnext_h * (1 - next_h**2)
    dx = dat.dot(Wx.T)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h_ = h0
    for i in range(T):
        h_, cache_ = rnn_step_forward(x[:,i,:], h_, Wx, Wh, b)
        h.append(h_)
        cache.append(cache_)
    h = np.stack(h, axis=1)
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dh_ = dh[:, t, :] + dh_next
        dx_t, dh_next, dWx_t, dWh_t, db_t = rnn_step_backward(dh_, cache[t])
        dx[:, t, :] += dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    next_c = np.multiply(f, prev_c) + np.multiply(i, g) # N, H
    next_h = np.multiply(o, np.tanh(next_c))

    cache = (x, prev_h, prev_c, Wx, Wh, b, a, ai, af, ao, ag, i, f, o, g, next_h, next_c)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, prev_h, prev_c, Wx, Wh, b, a, ai, af, ao, ag, i, f, o, g, next_h, next_c = cache
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dh_ = dh[:, t, :] + dh_next
        dx_t, dh_next, dc_next, dWx_t, dWh_t, db_t = lstm_step_backward(dh_, dc_next, cache[t])
        dx[:, t, :] += dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
