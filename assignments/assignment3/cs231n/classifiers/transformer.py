...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        embed_captions = self.embedding(captions) # N, T, W
        pos_embed_captions = self.positional_encoding(embed_captions) # N, T, W

        proj_features = self.visual_projection(features) # N, W
        memory = proj_features.unsqueeze(1) # N, 1, W

        tgt_mask = torch.tril(torch.ones(T, T, device=captions.device) * float('-inf'), diagonal=1)
        
        decoder_output = self.transformer(pos_embed_captions, memory, tgt_mask)
