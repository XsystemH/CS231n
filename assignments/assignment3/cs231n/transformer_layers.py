...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pe[0, :, 0::2] = torch.sin(position * div_term)  # 偶数索引使用正弦函数
        pe[0, :, 1::2] = torch.cos(position * div_term)  # 奇数索引使用余弦函数
    ...
    def forward(self, x):
        ...
        # 从预先计算的位置编码中选择与输入序列长度匹配的部分
        # 并确保位置编码在与输入相同的设备上（CPU 或 GPU）
        pos_encoding = self.pe[:, :seq_len, :].to(x.device)
    
        # 将位置编码添加到输入嵌入中，并应用 dropout
        output = self.dropout(x + pos_encoding)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        Q = self.query(query).view(N, S, H, self.head_dim).transpose(1, 2)
        K = self.key(key).view(N, T, H, self.head_dim).transpose(1, 2)
        V = self.value(value).view(N, T, H, self.head_dim).transpose(1, 2)
